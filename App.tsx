...
const Stopwatch: React.FC<{ aiAction: { action: string, timestamp: number } | null }> = ({ aiAction }) => {
    const [stopwatchTime, setStopwatchTime] = useState(() => loadFromLocalStorage('stopwatchTime', 0));
    const [stopwatchRunning, setStopwatchRunning] = useState(() => loadFromLocalStorage('stopwatchRunning', false));
    const [laps, setLaps] = useState<number[]>(() => loadFromLocalStorage('laps', []));
    ...
    useEffect(() => { saveToLocalStorage('stopwatchTime', stopwatchTime); }, [stopwatchTime]);
    useEffect(() => { saveToLocalStorage('stopwatchRunning', stopwatchRunning); }, [stopwatchRunning]);
    useEffect(() => { saveToLocalStorage('laps', laps); }, [laps]);
    ...
    const handleStopwatchReset = useCallback(() => {
        ...
        setStopwatchTime(0);
        setLaps([]);
        saveToLocalStorage('stopwatchTime', 0);
        saveToLocalStorage('stopwatchRunning', false);
    ...
    }, [aiAction, stopwatchRunning, handleStopwatchStartStop, handleStopwatchLap, handleStopwatchReset]);
...
const MonthDaySelector: React.FC<{ selectedDay: number; onDaySelect: (day: number) => void; maxDay?: number; }> = ({ selectedDay, onDaySelect, maxDay = 31 }) => {
    const days = Array.from({ length: maxDay }, (_, i) => i + 1);
    return (
        <div className="grid grid-cols-7 gap-1 p-2 bg-gray-700/50 rounded-md">
            {days.map(day => (
                <button key={day} type="button" onClick={() => onDaySelect(day)} className={`flex items-center justify-center w-8 h-8 text-xs rounded-full ${selectedDay === day ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-200 hover:bg-blue-900/40'}`} >
                    {day}
                </button>
            ))}
        </div>
...
const MonthSelector: React.FC<{ selectedMonth: number; onMonthSelect: (month: number) => void; }> = ({ selectedMonth, onMonthSelect }) => (
    <div className="grid grid-cols-4 gap-2 p-2 bg-gray-700/50 rounded-md">
        {MONTHS.map((month, index) => (
            <button key={month} type="button" onClick={() => onMonthSelect(index)} className={`px-2 py-2 text-xs rounded-md ${selectedMonth === index ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-200 hover:bg-blue-900/40'}`} >
                {month.substring(0, 3)}
            </button>
        ))}
    </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <form onSubmit={handleAddItem} className="flex gap-2">
        <input type="text" value={newItemText} onChange={(e) => setNewItemText(e.target.value)} placeholder="Add a new item..." className="flex-grow w-full px-3 py-2 text-sm bg-gray-700 text-gray-100 border border-transparent rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none" />
        <button type="submit" className="flex-shrink-0 p-2 text-white bg-blue-500 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"><PlusIcon /></button>
      </form>
      <ul className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
        {items.map((item) => (
          <li key={item.id} className="flex items-center gap-2 group">
            <input type="checkbox" checked={item.completed} onChange={() => onToggleItem(item.id)} className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500" />
            <span className={`flex-grow text-sm ${item.completed ? 'line-through text-gray-400' : 'text-gray-300'}`}>{item.text}</span>
            <button onClick={() => onDeleteItem(item.id)} className="text-gray-400 opacity-0 group-hover:opacity-100 hover:text-red-500 disabled:opacity-0" disabled={item.isRecurring}><TrashIcon /></button>
          </li>
        ))}
      </ul>
    </div>
...
const RemindersSection: React.FC<RemindersSectionProps> = ({ allRemindersForSelectedDate, handleReminderUpdate, selectedDateKey }) => {
    ...
    const handleAdd = (e: React.FormEvent) => {
        ...
        if (finalRecurrence.type !== 'weekly') delete finalRecurrence.daysOfWeek;
        if (finalRecurrence.type !== 'monthly' && finalRecurrence.type !== 'yearly') delete finalRecurrence.dayOfMonth;
        if (finalRecurrence.type !== 'yearly') delete finalRecurrence.monthOfYear;
        if (finalRecurrence.type === 'weekly' && (!finalRecurrence.daysOfWeek || finalRecurrence.daysOfWeek.length === 0)) {
            alert('Please select at least one day for weekly recurrence.');
            return;
        }
        handleReminderUpdate('add', { id: Date.now().toString(), text: text.trim(), time, completed: false, recurrence: finalRecurrence, completedDates: [] });
        setText('');
    ...
    return (
        <div className="space-y-3">
            ...
            <form onSubmit={handleAdd} className="p-2 -m-2 space-y-3 border border-transparent focus-within:border-gray-700 focus-within:bg-gray-900/20 rounded-lg">
                <div className="flex gap-2">
                    <input type="text" value={text} onChange={e => setText(e.target.value)} placeholder="New reminder..." className="flex-grow w-full px-3 py-2 text-sm bg-gray-700 text-gray-100 border border-transparent rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                    <input type="time" value={time} onChange={e => setTime(e.target.value)} className="px-3 py-2 text-sm bg-gray-700 text-gray-100 border border-transparent rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                </div>
                <div className="flex gap-2 items-start">
                    <select value={recurrence.type} onChange={e => setRecurrence({ type: e.target.value as any, daysOfWeek: [], dayOfMonth: 1, monthOfYear: 0 })} className="flex-grow w-full px-3 py-2 text-sm bg-gray-700 text-gray-100 border border-transparent rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    ...
                    <button type="submit" className="flex-shrink-0 p-2 text-white bg-blue-500 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 mt-auto"><PlusIcon /></button>
                </div>
                {recurrence.type === 'weekly' && (
                    <div className="p-2 bg-gray-700/50 rounded-md">
                        <p className="text-xs font-medium text-gray-400 mb-2">Repeat on:</p>
                        <div className="flex justify-around">
                            {WEEK_DAYS.map((day, index) => (
                                <button key={day} type="button" onClick={() => handleWeeklyDayToggle(index)} className={`w-8 h-8 text-xs rounded-full ${recurrence.daysOfWeek?.includes(index) ? 'bg-blue-500 text-white' : 'bg-gray-700 hover:bg-blue-900/40'}`}>{day.charAt(0)}</button>
                            ))}
                        </div>
                    </div>
                )}
                {recurrence.type === 'monthly' && (
                    <div className="space-y-2">
                        <p className="text-xs font-medium text-gray-400 px-2">Day of month:</p>
                        <MonthDaySelector selectedDay={recurrence.dayOfMonth || 1} onDaySelect={day => setRecurrence(prev => ({ ...prev, dayOfMonth: day }))} />
                    </div>
                )}
                {recurrence.type === 'yearly' && (
                    <div className="space-y-2">
                        <p className="text-xs font-medium text-gray-400 px-2">Month:</p>
                        <MonthSelector selectedMonth={recurrence.monthOfYear || 0} onMonthSelect={month => setRecurrence(prev => ({ ...prev, monthOfYear: month }))} />
                        <p className="text-xs font-medium text-gray-400 px-2">Day of month:</p>
                        <MonthDaySelector selectedDay={recurrence.dayOfMonth || 1} onDaySelect={day => setRecurrence(prev => ({ ...prev, dayOfMonth: day }))} />
                    </div>
                )}
            ...
            <ul className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                {allRemindersForSelectedDate.map(r => {
                    const isRecurringInstance = r.id.includes('-recurring-');
                    const isCompleted = isRecurringInstance ? (r.completedDates || []).includes(selectedDateKey) : r.completed;
                    return (
                        <li key={r.id} className="flex items-center gap-2 group">
                            <input type="checkbox" checked={isCompleted} onChange={() => handleReminderUpdate('toggle', r)} className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500" />
                            <div className="flex-grow">
                                <span className={`text-sm ${isCompleted ? 'line-through text-gray-400' : 'text-gray-300'}`}>{r.text}</span>
                                <span className="block text-xs text-gray-500">{formatTime12Hour(r.time)}</span>
                            </div>
                            {!isRecurringInstance && <button onClick={() => handleReminderUpdate('delete', r.id)} className="text-gray-400 opacity-0 group-hover:opacity-100 hover:text-red-500"><TrashIcon /></button>}
                        </li>
                    )
                })}
            </ul>
...
interface NoteSectionProps { note: Note | null; onUpdate: (content: string) => void; placeholder: string; }
const NoteSection: React.FC<NoteSectionProps> = ({ note, onUpdate, placeholder }) => {
    ...
    return (
        <div className="space-y-3">
            ...
            <textarea
                ...
                className="w-full h-24 px-3 py-2 text-sm bg-gray-700 text-gray-100 border border-transparent rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none"
            />
        </div>
    );
};
...
const CalendarHeader: React.FC<CalendarHeaderProps> = ({ mainView, currentDate, selectedDate, navigate, setCurrentDate, setSelectedDate, setMainView }) => {
    const title = useMemo(() => {
        switch (mainView) {
            case 'year': return currentDate.getFullYear();
            case 'month': return new Intl.DateTimeFormat('en-US', { month: 'long', year: 'numeric' }).format(currentDate);
            case 'week':
                const startOfWeek = getWeekDays(currentDate)[0];
                const endOfWeek = getWeekDays(currentDate)[6];
                return `${startOfWeek.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} - ${endOfWeek.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`;
            case 'day': return selectedDate.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
        }
    }, [mainView, currentDate, selectedDate]);
    return (
        <header className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
                <div className="flex items-center gap-1 p-1 bg-gray-700 rounded-lg">
                    {(['day', 'week', 'month', 'year'] as MainView[]).map(view => (
                        <button key={view} onClick={() => setMainView(view)} className={`px-3 py-1 text-sm font-semibold rounded-md capitalize ${mainView === view ? 'bg-gray-800 text-blue-400 shadow-sm' : 'text-gray-300 hover:bg-gray-600'}`}>{view}</button>
                    ))}
                </div>
                <div className="flex items-center gap-2">
                    <button onClick={() => navigate(-1)} className="p-2 rounded-full hover:bg-gray-700"><ChevronLeftIcon /></button>
                    <button onClick={() => { setCurrentDate(new Date()); setSelectedDate(new Date()); }} className="px-4 py-2 text-sm font-semibold border border-gray-600 rounded-md hover:bg-gray-700">Today</button>
                    <button onClick={() => navigate(1)} className="p-2 rounded-full hover:bg-gray-700"><ChevronRightIcon /></button>
                </div>
            </div>
        </header>
    );
...
const DayView: React.FC<Pick<ViewProps, 'selectedDate' | 'calendarData' | 'alarms' | 'holidays'>> = ({ selectedDate, calendarData, alarms, holidays }) => {
    const dayReminders = getRemindersForDate(selectedDate, calendarData);
    const dayAlarms = getAlarmsForDate(selectedDate, alarms);
    const dayHolidays = holidays[formatDate(selectedDate)] || [];
    const hours = Array.from({ length: 16 }, (_, i) => i + 7);
    const events = [
        ...dayReminders.map(r => ({ ...r, type: 'reminder' as const })),
        ...dayAlarms.map(a => ({ ...a, id: a.id, text: a.label, type: 'alarm' as const })),
        ...dayHolidays.map(h => ({ ...h, time: 'all-day', type: 'holiday' as const }))
    ].sort((a, b) => a.time.localeCompare(b.time));
    return (
        <div className="min-h-full bg-gray-800">
            <div className="relative pl-16">
                <div className="absolute top-0 left-8 w-px h-full bg-gray-700"></div>
                {hours.map(hour => (
                    <div key={hour} className="relative h-16">
                        <div className="absolute -left-2 top-0 pr-4 text-xs text-right w-16 text-gray-500">{hour % 12 || 12} {hour < 12 || hour === 24 ? 'AM' : 'PM'}</div>
                        <div className="absolute top-0 left-8 w-full border-t border-gray-700"></div>
                    </div>
                ))}
                {events.map((event, i) => {
                    if (event.type === 'holiday') return <div key={`${event.name}-${i}`} className="absolute -top-1 left-16 right-0 p-2 text-sm font-semibold text-center bg-green-900/50 text-green-300 rounded-lg">{event.name}</div>;
                    const [h, m] = event.time.split(':').map(Number);
                    const top = (h - 7 + m / 60) * 4;
                    const isAlarm = event.type === 'alarm';
                    return (
                        <div key={event.id} style={{ top: `${top}rem` }} className={`absolute left-20 right-4 p-2 rounded-r-lg ${isAlarm ? 'bg-red-900/50 border-l-4 border-red-500' : 'bg-blue-900/50 border-l-4 border-blue-500'}`}>
                            <p className={`text-sm font-semibold ${isAlarm ? 'text-red-200' : 'text-blue-200'}`}>{event.text}</p>
                            <p className={`text-xs ${isAlarm ? 'text-red-400' : 'text-blue-400'}`}>{formatTime12Hour(event.time)}</p>
                        </div>
                    );
                })}
            </div>
...
const WeekView: React.FC<Pick<ViewProps, 'currentDate' | 'calendarData' | 'alarms' | 'holidays' | 'handleDateClick' | 'recurringChecklistItems'>> = ({ currentDate, calendarData, alarms, holidays, handleDateClick, recurringChecklistItems }) => {
    const weekDates = getWeekDays(currentDate);
    return (
        <div className="flex flex-col h-full border-t border-gray-700">
            <div className="grid grid-cols-7 border-l border-gray-700 flex-shrink-0">
                {weekDates.map(day => (
                    <div key={day.toISOString()} className="p-2 text-center font-semibold text-sm text-gray-300 border-r border-b border-gray-700">
                        {day.toLocaleDateString('en-US', { weekday: 'short' })} <span className="text-gray-400">{day.getDate()}</span>
                    </div>
                ))}
            </div>
            <div className="grid grid-cols-7 flex-grow border-l border-gray-700 h-0">
                {weekDates.map(day => {
                    const dayKey = formatDate(day);
                    const dayData = calendarData[dayKey];
                    const dayReminders = getRemindersForDate(day, calendarData);
                    const dayAlarms = getAlarmsForDate(day, alarms);
                    const dayHolidays = holidays[dayKey] || [];
                    const dayNote = dayData?.note;
                    const completedIds = new Set(dayData?.completedRecurringItemIds || []);
                    const dayRecurringItems = recurringChecklistItems.filter(item => {
                        const { type, daysOfWeek, dayOfMonth, monthOfYear } = item.recurrence;
                        if (type === 'daily') return true;
                        if (type === 'weekly' && daysOfWeek?.includes(day.getDay())) return true;
                        if (type === 'monthly' && dayOfMonth === day.getDate()) return true;
                        if (type === 'yearly' && monthOfYear === day.getMonth() && dayOfMonth === day.getDate()) return true;
                        return false;
                    });
                    const dailyCombinedChecklistItems = [...(dayRecurringItems.map(item => ({ ...item, completed: completedIds.has(item.id), isRecurring: true }))), ...((dayData?.checklist || []).map(item => ({...item, isRecurring: false })))];
                    return (
                        <div key={day.toISOString()} onClick={() => handleDateClick(day)} className="relative flex flex-col border-r border-gray-700 cursor-pointer hover:bg-blue-900/20">
                            <div className="flex-grow overflow-y-auto p-2 space-y-2 custom-scrollbar">
                                {dayHolidays.map(h => (<div key={h.name} className="p-1 text-xs text-center bg-green-900/50 text-green-300 rounded-md">{h.name}</div>))}
                                {dayAlarms.map(a => (<div key={a.id} className="p-1 text-xs text-white bg-red-500 rounded-md"><p className="font-semibold truncate flex items-center gap-1"><AlarmIcon/>{a.label}</p><p>{formatTime12Hour(a.time)}</p></div>))}
                                {dayReminders.map(r => (<div key={r.id} className="p-1 text-xs text-white bg-blue-500 rounded-md"><p className="font-semibold truncate">{r.text}</p><p>{formatTime12Hour(r.time)}</p></div>))}
                                {dayNote?.content && (<div><h4 className="text-xs font-bold text-gray-400 mt-2">Note</h4><p className="text-xs text-gray-300 bg-gray-700/50 p-1 rounded whitespace-pre-wrap break-words">{dayNote.content}</p></div>)}
                                {dailyCombinedChecklistItems.length > 0 && (<div><h4 className="text-xs font-bold text-gray-400 mt-2">Checklist</h4><ul className="space-y-1">{dailyCombinedChecklistItems.map(item => (<li key={item.id} className="flex items-center gap-1.5"><input type="checkbox" checked={item.completed} readOnly className="w-3 h-3 text-blue-600 bg-gray-600 border-gray-500 rounded focus:ring-blue-500 pointer-events-none" /><span className={`text-xs ${item.completed ? 'line-through text-gray-500' : 'text-gray-300'}`}>{item.text}</span></li>))}</ul></div>)}
                            </div>
                        </div>
                    );
                })}
...
const MonthView: React.FC<ViewProps> = ({ currentDate, selectedDate, mainView, calendarData, holidays, selectedDateKey, handleDateClick }) => {
    ...
    return (
        <div className="grid grid-cols-7 gap-px text-xs font-semibold text-center text-gray-400 bg-gray-700 border border-gray-700">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (<div key={day} className="py-2 bg-gray-800">{day}</div>))}
            {calendarGrid.map((day, index) => {
                if (!day) return <div key={index} className="bg-gray-800/50"></div>;
                const dayKey = formatDate(day);
                const isToday = dayKey === formatDate(new Date());
                const isSelected = dayKey === selectedDateKey;
                const dayReminders = getRemindersForDate(day, calendarData);
                const dayHolidays = holidays[dayKey] || [];
                const hasData = dayReminders.length > 0 || (calendarData[dayKey] && (calendarData[dayKey].checklist.length > 0 || calendarData[dayKey].note));
                return (
                    <div key={index} className={`relative flex flex-col p-2 bg-gray-800 h-24 md:h-32 lg:h-40 overflow-hidden ${day ? 'cursor-pointer hover:bg-blue-900/20' : ''}`} onClick={() => handleDateClick(day)}>
                        <span className={`self-end w-7 h-7 flex items-center justify-center rounded-full text-sm ${isToday ? 'bg-blue-500 text-white' : ''} ${isSelected && mainView === 'day' ? 'ring-2 ring-blue-500' : ''}`}>{day.getDate()}</span>
                        {hasData && <div className="absolute w-1.5 h-1.5 bg-blue-500 rounded-full top-2 left-2"></div>}
                        <div className="mt-1 space-y-1 overflow-hidden">
                            {dayHolidays.map(h => (<div key={h.name} className="px-1 py-0.5 text-xs bg-green-900/50 text-green-300 rounded-sm truncate">{h.name}</div>))}
                            {dayReminders.slice(0, 2).map(r => (<div key={r.id} className="px-1 py-0.5 text-xs text-white bg-blue-400 rounded-sm truncate">{r.text}</div>))}
                        </div>
                    </div>
                );
            })}
...
const TopBar: React.FC<{ appView: AppView; setAppView: (v: AppView) => void; setIsAiModalOpen: (isOpen: boolean) => void; }> = ({ appView, setAppView, setIsAiModalOpen }) => (
    <div className="flex items-center justify-between p-4 bg-gray-800 border-b border-gray-700">
        ...
        <div className="flex-1 flex items-center justify-center gap-2 md:gap-4">
            {(['calendar', 'alarms', 'timers', 'stopwatch'] as AppView[]).map(view => (
...
const AlarmsView: React.FC<{
...
}> = ({ alarms, setAlarms, ringingAlarms, setRingingAlarms }) => {
    ...
    const handleAddAlarm = (e: React.FormEvent) => {
        e.preventDefault();
        if (!label.trim() || (repeat && days.length === 0)) {
            alert('Please provide a label. Select at least one day for repeating alarms.');
            return;
        }
        ...
        setAlarms(prev => [...prev, { id: Date.now().toString(), time, label, days: repeat ? days : [], enabled: true, isOneTime: !repeat, targetDate }]);
        setLabel('');
        setDays([]);
        setRepeat(false);
    };
...
const TimersView: React.FC<{
    ...
    ringingTimers: Set<string>;
    setRingingTimers: React.Dispatch<React.SetStateAction<Set<string>>>;
}> = ({ timers, setTimers, ringingTimers, setRingingTimers }) => {
    const [label, setLabel] = useState('');
...
const StopwatchView: React.FC<{ aiStopwatchAction: { action: string, timestamp: number } | null }> = ({ aiStopwatchAction }) => {
...
const CalendarView: React.FC<{
    ...
    alarms: Alarm[];
    holidays: { [dateKey: string]: Holiday[] };
    selectedDateKey: string;
    recurringChecklistItems: RecurringChecklistItem[];
    handleDateClick: (date: Date) => void;
    handleMonthClick: (month: number) => void;
    sidebarContent: MainView | 'recurring';
    handleSidebarTabClick: (view: MainView | 'recurring') => void;
    allRemindersForSelectedDate: (Reminder & { originalDateKey?: string; })[];
    handleReminderUpdate: (action: 'add' | 'toggle' | 'delete', payload: any) => void;
    selectedDateData: DateData;
    handleNoteUpdate: (content: string) => void;
    dailyCombinedChecklistItems: (ChecklistItem & { isRecurring?: boolean; })[];
    handleDailyChecklistUpdate: (action: 'add' | 'toggle' | 'delete', payload: any) => void;
    currentWeekNote: Note | null;
    handleWeeklyNoteUpdate: (content: string) => void;
    currentWeekChecklist: ChecklistItem[];
    weeklyChecklistHandlers: { onAddItem: (text: string) => void; onToggleItem: (id: string) => void; onDeleteItem: (id: string) => void; };
    currentMonthNote: Note | null;
    handleMonthlyNoteUpdate: (content: string) => void;
    currentMonthChecklist: ChecklistItem[];
    monthlyChecklistHandlers: { onAddItem: (text: string) => void; onToggleItem: (id: string) => void; onDeleteItem: (id: string) => void; };
    currentYearNote: Note | null;
    handleYearlyNoteUpdate: (content: string) => void;
    currentYearChecklist: ChecklistItem[];
    yearlyChecklistHandlers: { onAddItem: (text: string) => void; onToggleItem: (id: string) => void; onDeleteItem: (id: string) => void; };
    recurringChecklistHandlers: { onAddItem: (text: string, recurrence: ChecklistRecurrenceRule) => void; onDeleteItem: (id: string) => void; };
}> = (props) => {
    const { 
        mainView, currentDate, selectedDate, navigate, setCurrentDate, setSelectedDate, setMainView, calendarData, alarms, holidays, 
        selectedDateKey, recurringChecklistItems, handleDateClick, handleMonthClick, sidebarContent, handleSidebarTabClick,
        allRemindersForSelectedDate, handleReminderUpdate, selectedDateData, handleNoteUpdate, dailyCombinedChecklistItems,
        handleDailyChecklistUpdate, currentWeekNote, handleWeeklyNoteUpdate, currentWeekChecklist, weeklyChecklistHandlers,
        currentMonthNote, handleMonthlyNoteUpdate, currentMonthChecklist, monthlyChecklistHandlers, currentYearNote,
        handleYearlyNoteUpdate, currentYearChecklist, yearlyChecklistHandlers, recurringChecklistHandlers
    } = props;
      ...
      <div className="flex h-full bg-gray-900/50 text-gray-200 relative">
        <main className="flex-1 flex flex-col bg-gray-800 shadow-sm overflow-hidden">
          <div className="p-4 md:p-6 lg:p-8 border-b border-gray-700">
            <CalendarHeader mainView={mainView} currentDate={currentDate} selectedDate={selectedDate} navigate={navigate} setCurrentDate={setCurrentDate} setSelectedDate={setSelectedDate} setMainView={setMainView}/>
          </div>
        ...
        <aside className="custom-scrollbar w-full md:w-96 lg:w-[400px] bg-gray-800 border-l border-gray-700 flex-shrink-0 p-6 space-y-6 overflow-y-auto absolute md:relative h-full inset-0 z-10 md:z-auto">
          ...
          <div className="space-y-6">
              {sidebarContent === 'day' && (<div className="space-y-6"><RemindersSection allRemindersForSelectedDate={allRemindersForSelectedDate} handleReminderUpdate={handleReminderUpdate} selectedDateKey={selectedDateKey} /><NoteSection note={selectedDateData.note} onUpdate={handleNoteUpdate} placeholder="Add a note for this day..."/><Checklist title="Checklist" items={dailyCombinedChecklistItems} onAddItem={(text) => handleDailyChecklistUpdate('add', text)} onToggleItem={(id) => handleDailyChecklistUpdate('toggle', id)} onDeleteItem={(id) => handleDailyChecklistUpdate('delete', id)}/></div>)}
              {sidebarContent === 'week' && (<div className="space-y-6"><NoteSection note={currentWeekNote} onUpdate={handleWeeklyNoteUpdate} placeholder="Add a note for this week..."/><Checklist title="Weekly Checklist" items={currentWeekChecklist} onAddItem={weeklyChecklistHandlers.onAddItem} onToggleItem={weeklyChecklistHandlers.onToggleItem} onDeleteItem={weeklyChecklistHandlers.onDeleteItem}/></div>)}
              {sidebarContent === 'month' && (<div className="space-y-6"><NoteSection note={currentMonthNote} onUpdate={handleMonthlyNoteUpdate} placeholder="Add a note for this month..."/><Checklist title="Monthly Checklist" items={currentMonthChecklist} onAddItem={monthlyChecklistHandlers.onAddItem} onToggleItem={monthlyChecklistHandlers.onToggleItem} onDeleteItem={monthlyChecklistHandlers.onDeleteItem}/></div>)}
              {sidebarContent === 'year' && (<div className="space-y-6"><NoteSection note={currentYearNote} onUpdate={handleYearlyNoteUpdate} placeholder="Add a note for this year..."/><Checklist title="Yearly Checklist" items={currentYearChecklist} onAddItem={yearlyChecklistHandlers.onAddItem} onToggleItem={yearlyChecklistHandlers.onToggleItem} onDeleteItem={yearlyChecklistHandlers.onDeleteItem}/></div>)}
...
const App: React.FC = () => {
  const [currentDate, setCurrentDate] = useState(() => {
      const savedDate = loadFromLocalStorage<string | null>('currentDate', null);
      return savedDate ? new Date(savedDate) : new Date();
  });
  const [selectedDate, setSelectedDate] = useState(() => {
      const savedDate = loadFromLocalStorage<string | null>('selectedDate', null);
      return savedDate ? new Date(savedDate) : new Date();
  });
  const [mainView, setMainView] = useState<MainView>(() => loadFromLocalStorage('mainView', 'month'));
  const [appView, setAppView] = useState<AppView>(() => loadFromLocalStorage('appView', 'calendar'));
  const [sidebarContent, setSidebarContent] = useState<MainView | 'recurring'>(() => loadFromLocalStorage('sidebarContent', 'month'));
  const [calendarData, setCalendarData] = useState<CalendarData>(() => loadFromLocalStorage('calendarData', {}));
  const [monthlyChecklists, setMonthlyChecklists] = useState<MonthlyChecklists>(() => loadFromLocalStorage('monthlyChecklists', {}));
  const [weeklyChecklists, setWeeklyChecklists] = useState<WeeklyChecklists>(() => loadFromLocalStorage('weeklyChecklists', {}));
  const [yearlyChecklists, setYearlyChecklists] = useState<YearlyChecklists>(() => loadFromLocalStorage('yearlyChecklists', {}));
  const [recurringChecklistItems, setRecurringChecklistItems] = useState<RecurringChecklistItem[]>(() => loadFromLocalStorage('recurringChecklistItems', []));
  const [weeklyNotes, setWeeklyNotes] = useState<WeeklyNotes>(() => loadFromLocalStorage('weeklyNotes', {}));
  const [monthlyNotes, setMonthlyNotes] = useState<MonthlyNotes>(() => loadFromLocalStorage('monthlyNotes', {}));
  const [yearlyNotes, setYearlyNotes] = useState<YearlyNotes>(() => loadFromLocalStorage('yearlyNotes', {}));
  const [holidays, setHolidays] = useState<{ [dateKey: string]: Holiday[] }>({});
  const [alarms, setAlarms] = useState<Alarm[]>(() => loadFromLocalStorage('alarms', []));
  const [timers, setTimers] = useState<Timer[]>(() => loadFromLocalStorage('timers', []));
  ...
  // --- PERSISTENCE EFFECTS ---
  useEffect(() => { saveToLocalStorage('currentDate', currentDate.toISOString()); }, [currentDate]);
  useEffect(() => { saveToLocalStorage('selectedDate', selectedDate.toISOString()); }, [selectedDate]);
  useEffect(() => { saveToLocalStorage('mainView', mainView); }, [mainView]);
  useEffect(() => { saveToLocalStorage('appView', appView); }, [appView]);
  useEffect(() => { saveToLocalStorage('sidebarContent', sidebarContent); }, [sidebarContent]);
  useEffect(() => { saveToLocalStorage('calendarData', calendarData); }, [calendarData]);
  useEffect(() => { saveToLocalStorage('monthlyChecklists', monthlyChecklists); }, [monthlyChecklists]);
  useEffect(() => { saveToLocalStorage('weeklyChecklists', weeklyChecklists); }, [weeklyChecklists]);
  useEffect(() => { saveToLocalStorage('yearlyChecklists', yearlyChecklists); }, [yearlyChecklists]);
  useEffect(() => { saveToLocalStorage('recurringChecklistItems', recurringChecklistItems); }, [recurringChecklistItems]);
  useEffect(() => { saveToLocalStorage('weeklyNotes', weeklyNotes); }, [weeklyNotes]);
  useEffect(() => { saveToLocalStorage('monthlyNotes', monthlyNotes); }, [monthlyNotes]);
  useEffect(() => { saveToLocalStorage('yearlyNotes', yearlyNotes); }, [yearlyNotes]);
  useEffect(() => { saveToLocalStorage('alarms', alarms); }, [alarms]);
  useEffect(() => { saveToLocalStorage('timers', timers); }, [timers]);
    ...
    switch (name) {
        case 'addReminder': {
            ...
            if (date && time && description) {
                const newReminder: Reminder = { id: Date.now().toString(), text: description, time, completed: false, recurrence: { type: 'none' }, completedDates: [] };
        ...
        case 'addAlarm': {
            ...
            if (time && label) {
                ...
                const newAlarm: Alarm = { id: Date.now().toString(), time, label, days: repeat ? (days || []) : [], enabled: true, isOneTime: !repeat, targetDate };
  ...
  const handleReminderUpdate = (action: 'add' | 'toggle' | 'delete', payload: any) => {
      if (action === 'toggle') {
          const { id, originalDateKey } = payload;
          const isRecurring = id.includes('-recurring-');
          const originalId = isRecurring ? id.split('-recurring-')[0] : id;
          setCalendarData(prev => {
              const sourceDateKey = isRecurring ? originalDateKey : selectedDateKey;
              const dateData = prev[sourceDateKey];
              if (!dateData) return prev;
              const newReminders = dateData.reminders.map(r => {
                  if (r.id === originalId) {
                      if (isRecurring) {
                          const completedDates = r.completedDates || [];
                          const isCompleted = completedDates.includes(selectedDateKey);
                          return { ...r, completedDates: isCompleted ? completedDates.filter(d => d !== selectedDateKey) : [...completedDates, selectedDateKey] };
                      } else {
              ...
              return { ...prev, [sourceDateKey]: { ...dateData, reminders: newReminders } };
          });
          return;
      }
      setCalendarData(prev => {
          const prevDateData = prev[selectedDateKey] || { reminders: [], note: null, checklist: [], completedRecurringItemIds: [] };
          let newReminders: Reminder[];
          switch (action) {
              case 'add':
                  newReminders = [...prevDateData.reminders, payload];
                  break;
              case 'delete':
                  newReminders = prevDateData.reminders.filter(r => r.id !== payload);
                  break;
              default:
                  newReminders = prevDateData.reminders;
          }
          return { ...prev, [selectedDateKey]: { ...prevDateData, reminders: newReminders } };
      });
  };
  
  const handleNoteUpdate = useCallback((content: string) => {
    setCalendarData(prev => {
        const currentData = { ...(prev[selectedDateKey] || { reminders: [], note: null, checklist: [], completedRecurringItemIds: [] }) };
        if (content) {
            const noteId = currentData.note?.id || `note-${selectedDateKey}`;
            currentData.note = { id: noteId, content };
        } else {
            currentData.note = null;
        }
        return { ...prev, [selectedDateKey]: currentData };
    });
  }, [selectedDateKey]);
  
  const handleWeeklyNoteUpdate = useCallback((content: string) => {
      setWeeklyNotes(prev => {
          if (content) {
              const noteId = prev[currentWeekKey]?.id || `note-${currentWeekKey}`;
              return { ...prev, [currentWeekKey]: { id: noteId, content } };
          } else {
              const newNotes = { ...prev };
              delete newNotes[currentWeekKey];
              return newNotes;
          }
      });
  }, [currentWeekKey]);

  const handleMonthlyNoteUpdate = useCallback((content: string) => {
      setMonthlyNotes(prev => {
          if (content) {
              const noteId = prev[currentMonthKey]?.id || `note-${currentMonthKey}`;
              return { ...prev, [currentMonthKey]: { id: noteId, content } };
          } else {
              const newNotes = { ...prev };
              delete newNotes[currentMonthKey];
              return newNotes;
          }
      });
  }, [currentMonthKey]);

  const handleYearlyNoteUpdate = useCallback((content: string) => {
      setYearlyNotes(prev => {
          if (content) {
              const noteId = prev[currentYearKey]?.id || `note-${currentYearKey}`;
              return { ...prev, [currentYearKey]: { id: noteId, content } };
          } else {
              const newNotes = { ...prev };
              delete newNotes[currentYearKey];
              return newNotes;
          }
      });
  }, [currentYearKey]);
  ...
  const calendarViewProps = {
    mainView, currentDate, selectedDate, navigate, setCurrentDate, setSelectedDate, setMainView: handleSetMainView, calendarData, alarms, holidays, 
    selectedDateKey, recurringChecklistItems, handleDateClick, handleMonthClick, sidebarContent, handleSidebarTabClick,
    allRemindersForSelectedDate, handleReminderUpdate, selectedDateData, handleNoteUpdate, dailyCombinedChecklistItems,
    handleDailyChecklistUpdate, currentWeekNote, handleWeeklyNoteUpdate, currentWeekChecklist, weeklyChecklistHandlers,
    currentMonthNote, handleMonthlyNoteUpdate, currentMonthChecklist, monthlyChecklistHandlers, currentYearNote,
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900 text-gray-200">
      <TopBar appView={appView} setAppView={setAppView} setIsAiModalOpen={setIsAiModalOpen} />
      <div className="flex-1 overflow-hidden">
        {appView === 'calendar' && <CalendarView {...calendarViewProps} />}
        {appView === 'alarms' && <AlarmsView alarms={alarms} setAlarms={setAlarms} ringingAlarms={ringingAlarms} setRingingAlarms={setRingingAlarms} />}
        {appView === 'timers' && <TimersView timers={timers} setTimers={setTimers} ringingTimers={ringingTimers} setRingingTimers={setRingingTimers} />}
        {appView === 'stopwatch' && <StopwatchView aiStopwatchAction={aiStopwatchAction} />}
      </div>
